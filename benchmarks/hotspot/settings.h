...
void Settings::parseArguments(int argc, char** argv)
{
    OptionMap parsers = {
        ...
        {'c', OptionPtr(new Range(grid_cols, 0, (uint64_t)std::numeric_limits<uint64_t>::max, "cols", "cols", "0"))},
        {'r', OptionPtr(new Range(grid_rows, 0, (uint64_t)std::numeric_limits<uint64_t>::max, "rows", "rows", "0"))},
        {'h', OptionPtr(new Range(pyramid_height, 0, (uint64_t)std::numeric_limits<uint64_t>::max, "step", "pyramid-height", "0"))},
        {'i', OptionPtr(new Range(total_iterations, 0, (uint64_t)std::numeric_limits<uint64_t>::max, "iterations", "total-iterations", "10"))},
    };
