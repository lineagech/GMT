...
void Settings::parseArguments(int argc, char** argv)
{
    OptionMap parsers = {
        ...
        {'w', OptionPtr(new Range(warp_size, 0, (uint64_t)std::numeric_limits<uint64_t>::max, "warp-size", "warp-size", "128"))},
        {'D', OptionPtr(new Range(divs, 0, (uint64_t)std::numeric_limits<uint64_t>::max, "divs", "divs", "8"))},
        {'i', OptionPtr(new Range(total_iterations, 0, (uint64_t)std::numeric_limits<uint64_t>::max, "total-iterations", "total-iterations", "10"))},
    };
